...
export class ConferenceEditorComponent implements OnInit, OnDestroy {
  ...
  createFormGroup() {
    ...
    this.conferenceForm.valueChanges.subscribe((data) => {
      ...
      this.conferenceEditorService.updateInfoFormValidity(
...
export const divisionsValidator: ValidatorFn = (
  ...
  const divisions = formGroup.get('divisions') as FormArray;
  const isValid = divisions.length === 0 || divisions.length === 2;
