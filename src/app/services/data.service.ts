...
export class DataService {
  ...
  getScheduleByWeek(week: number) {
    return this.http.get<Game[]>(`${this.baseUrl}/schedule/week/${week}`, {
  ...
  async addGame(game: AddGameRequest): Promise<Game> {
    return this.http
      .post<Game>(`${this.baseUrl}/schedule/add-game`, game, {
        headers: this.headers,
      })
      .toPromise();
  }
  ...
  fixSchedule(): Observable<number> {
    return this.http.post<number>(`${this.baseUrl}/schedule/fix`, null, {
      headers: this.headers,
    });
  }
  ...
  saveAlignmentToExcel(): Observable<any> {
    return this.http.get(`${this.baseUrl}/conferences/download`, {
      responseType: 'blob',
    });
  }
  ...
  isConferenceListValid(conferences: Conference[]): boolean {
    ...
    conferences.forEach((conf: Conference) => {
      if (
        conf.shortName.toLowerCase() !== 'independent' &&
        conf.fbs &&
        !this.isConferenceValid(conf)
      ) {
        response = false;
      }
    });
  ...
  getBowlList(): Observable<Bowl[]> {
    return this.http.get<Bowl[]>(`${this.baseUrl}/bowls`, {
      headers: this.headers,
    });
  }

  setBowlList(bowlList: Bowl[]): Observable<Bowl[]> {
    return this.http.put<any>(`${this.baseUrl}/bowls`, bowlList, {
      headers: this.headers,
    });
  }
  ...
  saveConferences(conferences: Conference[]): Observable<any> {
    return this.http.put<any>(`${this.baseUrl}/conferences`, conferences, {
      headers: this.headers,
    });
  }
}
